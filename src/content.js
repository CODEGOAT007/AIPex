...
$(document).ready(() => {
	...
	waitForElement("#aipex-drag-icon", function (element) {
		...
		// 将关闭图标添加到拖动图标中
		$dragIcon.append(closeIcon);

		// 鼠标悬停时显示关闭图标
		$dragIcon.on("mouseenter", function () {
			closeIcon.show();
		});

		// 鼠标移出时隐藏关闭图标
		$dragIcon.on("mouseleave", function () {
			closeIcon.hide();
		});

		// 点击关闭图标时隐藏整个 aipex-drag-icon 组件
		...
		$dragIcon.on("mousedown", function (e) {
			...
			// 设置 pointer-events 为 none，防止触发其他事件
			$dragIcon.css("pointer-events", "none");
		});
		...
		function handleMouseUp() {
			...
			// 恢复 pointer-events
			$dragIcon.css("pointer-events", "auto");
		}

		// 点击事件
		$dragIcon.on("click", function (e) {
			e.stopPropagation(); // 阻止事件冒泡
			openaipex();
		});
	});
